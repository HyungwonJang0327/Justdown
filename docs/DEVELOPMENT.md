...
## 폐기된 계획: 자동 후리가나 (2026-09-08)

MeCab/kuromoji 온디바이스 형태소 분석으로 한자에 《요미》를 자동 삽입하는
기능을 검토했으나 **폐기**했다. 정확도 한계가 엔진이 아니라 사전에서 오고
(IPADIC이면 `一人`=いちにん 오독 등, MeCab이든 kuromoji든 같은 사전이면 동일),
진짜 레버인 "개인 오버라이드 사전"은 서버가 없어 사용자 간 수정 공유가 불가하다.
검수 필요한 편의 기능 하나를 위해 네이티브 모듈·dev build 전환·iOS 전용 같은
큰 비용을 치를 만큼의 편익이 없다고 판단.
